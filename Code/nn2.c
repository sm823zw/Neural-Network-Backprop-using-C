...
struct NeuralNet* newNet(int n_layers, int n_neurons_per_layer[]){
    ...
    nn->delta = malloc((nn->n_layers-1)*sizeof(double**));
    ...
    for(int i=0;i<nn->n_layers-1;i++){
        ...
        for(int j=0;j<nn->n_neurons_per_layer[i]+1;j++){
            ...
            nn->delta[i][j] = malloc((nn->n_neurons_per_layer[i+1] + 1)*sizeof(double));
        }
    }
...
void back_propagation(struct NeuralNet* nn, char* activation_fun, double learning_rate, double momentum){
    ...
    for(int k=0;k<nn->n_layers-1;k++){
        for(int i=1;i<nn->n_neurons_per_layer[k]+1;i++){
            for(int j=1;j<nn->n_neurons_per_layer[k+1]+1;j++){
                nn->delta[k][i][j] = (learning_rate * nn->theta[k+1][j] * nn->out[k][i]) + (momentum * nn->delta[k][i][j]);
                nn->w[k][i][j] += nn->delta[k][i][j];
            }
        }
    }
}
...
int main(){
    ...
    int n_layers = 4;
    int n_neurons_per_layer[] = {N_DIMS, 32, 32, N_CLASSES};
    ...
    fprintf(file, "train_error,train_acc,test_error,test_acc\n");
    for(int itr=0;itr<epochs;itr++){
        ...
        fprintf(file, "%lf,", train_error);
        fprintf(file, "%lf,", train_acc);
        fprintf(file, "%lf,", test_error);
        fprintf(file, "%lf\n", test_acc);
